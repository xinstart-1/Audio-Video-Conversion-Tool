...
echo ==============================================================
echo   Voice to WAV  (.aud / .amr / .mpeg)
echo ==============================================================
echo   Folder : %CD%
echo   Mode   : recursive (includes sub-folders)
echo   Rule   : x.aud -^> x.aud.wav , then rename source to x.aud1
echo ==============================================================
echo.
...
rem Prefer the .py next to this .bat; otherwise use the embedded copy.
if exist "%~dpn0.py" goto :use_external
python -c "import sys;p=sys.argv[1];sys.argv=[p]+sys.argv[2:];exec(open(p,encoding='utf-8').read().rsplit('#=====EMBEDDED=====#',1)[1])" "%~f0" %*
goto :finish
